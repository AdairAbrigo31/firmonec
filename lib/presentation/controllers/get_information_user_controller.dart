...
class GetInformationUserController {
  static Future<void> executeActionsWithMicrosoft(WidgetRef ref) async {
    ...
    if (responseData != null) {
      ...
      print(
          "Su usuario es : ${stateUserProvider.email}, y token: ${stateUserProvider.token}");
    }
  ...
  static Future<void> executeActionWithVPN({required WidgetRef ref}) async {
    ...
    final repository = ref.read(repositoryProvider);
    print(
        "Usuario que hara la peticion: ${userProvider.email} con token: ${userProvider.token}");
    final List<RolEntity> roles = await repository.getRoles(
        email: userProvider.email!, token: userProvider.token);
    print(roles);
